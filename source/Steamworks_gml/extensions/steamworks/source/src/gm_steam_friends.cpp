...
void steam_friends_activate_game_overlay_to_store(std::uint32_t app_id, gm_enums::SteamFriendsOverlayToStoreFlag flag)
{
    ...
    f->ActivateGameOverlayToStore((AppId_t)app_id, (EOverlayToStoreFlag)(int)flag);
}
...
void steam_friends_activate_game_overlay_to_web_page(
    std::string_view url, gm_enums::SteamFriendsOverlayToWebpageMode mode
)
{
    STEAM_GUARD();
    ISteamFriends* f = steam_friends_iface();
...
std::uint64_t steam_friends_get_chat_member_by_index(std::uint64_t steam_id_clan, std::int32_t user)
{
    ...
    CSteamID id = f->GetChatMemberByIndex(steam_id_from_u64(steam_id_clan), user);
...
std::string steam_friends_get_friend_rich_presence_key_by_index(std::uint64_t steam_id_friend, std::int32_t key)
{
    ...
    const char* s = f->GetFriendRichPresenceKeyByIndex(steam_id_from_u64(steam_id_friend), key);
    return s ? std::string(s) : std::string();
}
