...
static struct json_object* json_object_new(enum json_type o_type)
{
  ...
  jso = (struct json_object*)yy_calloc(sizeof(struct json_object), 1);
  if(!jso) return NULL;
  jso->o_type = o_type;
  jso->_ref_count = 1;
			    ...
			    struct json_object *val)
{
  ...
  lh_table_insert(jso->o.c_object, yy_strdup(key), val);
}
					     ...
					     struct printbuf *pb)
{
	...
	if (isinf(jso->o.c_double)) {
		if (jso->o.c_double < 0)
			ret = sprintbuf(pb, "\"-infinity\"");
		else
			ret = sprintbuf(pb, "\"infinity\"");
	} // end if
	else
	if (isnan(jso->o.c_double)) {
		ret = sprintbuf(pb, "\"nan\"");
	} // end if
	else {
		int bpos = pb->bpos;
		ret = sprintbuf(pb, "%.32g", jso->o.c_double);
		// check for any '.' if not one then add it
		int fDot = FALSE;
		for (int n = bpos; !fDot && (n < pb->bpos); ++n) {
			fDot = (pb->buf[n] == '.');
		} // end for
		if (!fDot) sprintbuf(pb, ".0");
	} // end else
	return ret;
}
...
double json_object_get_double(struct json_object *jso)
{
  ...
  case json_type_boolean:
    return jso->o.c_boolean;
  case json_type_string:
    if(sscanf(jso->o.c_string, "%lg", &cdouble) == 1) return cdouble;
  default:
    return 0.0;
  }
}
...
{
	struct json_object *jso = json_object_new(json_type_null);
	if (!jso) return NULL;
