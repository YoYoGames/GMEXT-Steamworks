...
//#include "Files/Function/Function_Math.h"
...
#ifdef OS_Windows//#if !HAVE_STRNCASECMP && defined(_MSC_VER)
...
int utf8_get_charW(const char *str)
{
	int i = (int)*str;
	...
	if (i & 0x80)
	{
		...
		{	
			...
			{
				//4bytes
				i = ((i & 0x07) << 18) + (((*(str + 1)) & 0x3f) << 12) + (((*(str + 2)) & 0x3f) << 6) + ((*(str + 3)) & 0x3f);
			}
			else
			{
				// 3 bytes
				i = ((i & 0x0F) << 12) + (((*(str + 1)) & 0x3F) << 6) + ((*(str + 2)) & 0x3F);
			}
		}
		else
		{	// 2 bytes
			i = ((i & 0x1F) << 6) + ((*(str + 1)) & 0x3F);
		}
	}
...
struct json_tokener* json_tokener_new(void)
{
  ...
  tok = (struct json_tokener*)yy_calloc(1, sizeof(struct json_tokener));
  if (!tok) return NULL;
  tok->pb = printbuf_new();
...
static void json_tokener_reset_level(struct json_tokener *tok, int depth)
{
  ...
  yy_free(tok->stack[depth].obj_field_name);
...
char* strndup(const char* str, size_t n)
{
  if(str) {
    ...
    char* s = (char*)yy_malloc(sizeof(char) * (nn + 1));
...
#define ADVANCE_CHAR(str, tok) \
	( (str)+=utf8_char_sizeW(utf8_get_charW(str)), ((tok)->char_offset)+=utf8_char_sizeW(utf8_get_charW(str)) , c)
					  ...
					  const char *str, int len)
{
  ...
  redo_char:
	...
	printbuf_memappend_fast(tok->pb, (const char*)&c, 1);
    ...
    case json_tokener_state_null:
	  printbuf_memappend_fast(tok->pb, (const char*)&c, utf8_char_sizeW(c));
	...
	while(c && strchr(json_number_chars, c)) {
      ...
      }
      {
        ...
        if(!tok->is_double && (sscanf(tok->pb->buf, "%lld", &numi) == 1)) {
            unsigned long long top =((unsigned long long)numi & 0xffffffff80000000UL);
            if ( (top== 0x0) || (top == 0xffffffff80000000UL)) {
                current = json_object_new_int((int)numi);
            } else {
                current = json_object_new_double((double)numi);
            }
        } else if(tok->is_double && sscanf(tok->pb->buf, "%lg", &numd) == 1) {
	...
	while(1) {
	  if(c == tok->quote_char) {
	    printbuf_memappend_fast(tok->pb, case_start, str-case_start);
	    obj_field_name = yy_strdup(tok->pb->buf);
 ...
 out:
  ...
  printf("json_tokener_parse_ex: error %s at offset %d\n",
