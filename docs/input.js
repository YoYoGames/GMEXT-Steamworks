...
@param {boolean} explicitly_call_run_frame Are you going to call ${function.} manually or not?
...
@param {boolean} uses_steam_input_api Whether this device is using the Action Set and Actions API.
@param {boolean} uses_steam_gamepad_api Whether this device is using the XInput emulation API.
...
@param {boolean} active Whether or not this action is currently available to be bound in the active action set. If it is not available, OR does not belong to the active action set, this will be false.
@param {boolean} state **DIGITAL ACTIONS ONLY:** state of the digital action, `true` or `false`.
...
@desc This function will return a full path to a PNG file of the glyph image of an action origin. The path is automatically added into the GameMaker's filesystem sandbox list, so that you can use the returned path in ${function.sprite_add} or ${function.buffer_load} no matter if you have the sandbox enabled or disabled. The image will be in legacy Steam Big Picture design style, if you wish to use the new Steam Deck styled glyphs (also called Knockout glyphs in Valve's documentation), please use the function ${function.} instead.
