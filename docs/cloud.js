...
* [steam_is_cloud_enabled_for_app](#steam_is_cloud_enabled_for_app)
* [steam_is_cloud_enabled_for_account](#steam_is_cloud_enabled_for_account)
* [steam_get_quota_total](#steam_get_quota_total)
* [steam_get_quota_free](#steam_get_quota_free)
* [steam_file_exists](#steam_file_exists)
* [steam_file_size](#steam_file_size)
* [steam_file_persisted](#steam_file_persisted)
* [steam_file_write](#steam_file_write)
* [steam_file_write_file](#steam_file_write_file)
* [steam_file_read](#steam_file_read)
* [steam_file_share](#steam_file_share)
...
> **:warning: IMPORTANT**
>
> This does not automatically mean that you can use the Cloud functions as the user can switch off Cloud synchronization from their Steam Client. You can check this using the function [steam_is_cloud_enabled_for_account](#steam_is_cloud_enabled_for_account), but, even if it is disabled for the user (and enabled for the game), the functions will still work to store and retrieve data from a local copy of all files, it will just not upload them to the cloud on the game end, nor synchronize on the game start.
...
> **:warning: IMPORTANT**
>
> This does not automatically mean that you can store data to the Cloud, as it will also have to have been enabled for your game (you can check this using the function [steam_is_cloud_enabled_for_app](#steam_is_cloud_enabled_for_app)). If the Steam Cloud is enabled for your game, but the user has it switched off locally, you can still use the Cloud functions to store and retrieve data from a local copy of all files, it will just not upload them to the cloud on the game end, nor synchronize on the game start.
...
With this function you can find out how much free space is left for the user of the Steam Cloud quota. The value returned is in <i>bytes</i>.
...
With this function you can copy the contents of a locally saved file to a file that is synchronized with the Steam Cloud. The local file <i>must exist</i> before using this function, and it will return a value of 0 if it fails for whatever reason and a value greater than 0 if it succeeds.
